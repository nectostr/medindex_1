...
# !!!
pdata = pd.read_csv('data_2_try.csv', delimiter=',')
...
for k in range(0,AMOUNT_OF_ANALYSIS):
    ...
    names_of_columns.append(pdata.columns[2*k][0:pdata.columns[2*k].find('_')])
    ...
    for i in range(len(pdata)):
        ...
        # second_value = String
        # !!!
        first_value = pdata.iloc[i,k*2]
        second_value = pdata.iloc[i,k*2+1]
    ...
    print(amount_of_valid_analysis)
    ...
    if amount_of_valid_analysis != 0:
        ...
        for i in list_of_сurrent_deviation:
            if i > high_face_of_interval[k]:
                ...
                average_deviation[k] +=  i - low_face_of_interval[k]
...
# !!
x = np.asarray(average_deviation)

# Creating fake y values for good-looking drawing
y = np.asarray([i for i in range(0, len(x))])

# Preparing data for drawing interval from minimum to maximum deviation
lower_error = np.asarray(average_deviation) - np.asarray(minimum_deviation)
upper_error = np.asarray(maximum_deviation) - np.asarray(average_deviation)
